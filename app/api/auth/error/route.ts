...
export async function GET(req: NextRequest) {
  ...
  console.error(`[Auth Error Route] Error: ${error}, Code: ${code}, URL: ${req.url}`)
    ...
    <html lang="zh-CN">
      <head>
        ...
        <style>
          ...
          .card { background: #1e293b; border-radius: 12px; padding: 32px; max-width: 500px; width: 100%; box-shadow: 0 10px 25px rgba(0,0,0,0.5); }
          h1 { color: #ef4444; font-size: 20px; margin-top: 0; }
          p { color: #94a3b8; line-height: 1.6; }
          .error-box { background: #0f172a; border-left: 4px solid #ef4444; padding: 12px 16px; border-radius: 4px; font-family: monospace; color: #fca5a5; margin: 16px 0; word-break: break-all; }
      ...
      <body>
        <div class="card">
          ...
          <div class="error-box">Error: ${error} ${code ? `(Code: ${code})` : ''}</div>
          <p>请点击下方按钮返回重试：</p>
